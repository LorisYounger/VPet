...
namespace VPet_Simulator.Windows
{
	public partial class MainWindow : IMainWindow
	{
		...
		// 今日幸运色缓存，供后续唤起查看
		private string _todaysLuckyColorName = null;
		private Color? _todaysLuckyColor = null;

		private void ShowLuckyColor()
		{
			if (_todaysLuckyColorName == null || _todaysLuckyColor == null)
			{
				// 如果未生成则触发生成一次（与 GameLoad 的逻辑一致）
				GenerateAndApplyLuckyColor();
			}
			try
			{
				string title = "LuckyColor_Title".Translate();
				if (string.IsNullOrWhiteSpace(title) || title == "LuckyColor_Title")
				{
					var culture = LocalizeCore.CurrentCulture ?? System.Globalization.CultureInfo.CurrentUICulture.Name;
					if (culture.StartsWith("ja")) title = "本日のラッキーカラー";
					else if (culture.StartsWith("en")) title = "Today's Lucky Color";
					else if (culture.StartsWith("zh-Hant")) title = "今日幸運色";
					else title = "今日幸运色";
				}
                // 优先从本地化文件获取模板并直接格式化（支持占位符 {0} 等）
                string msg = null;
                var hostName = GameSavesData?.GameSave?.HostName;
                // 尝试以颜色名为第一个参数、主人名为第二个参数去翻译
                if (!string.IsNullOrWhiteSpace(_todaysLuckyColorName))
                {
                    try
                    {
                        if (!string.IsNullOrWhiteSpace(hostName))
                            msg = "LuckyColor_Message".Translate(_todaysLuckyColorName, hostName);
                        else
                            msg = "LuckyColor_Message".Translate(_todaysLuckyColorName);
                    }
                    catch
                    {
                        msg = null;
                ...
                if (string.IsNullOrWhiteSpace(msg) || msg == "LuckyColor_Message")
                {
					// 根据语言挑选更有桌宠风格的模板
					var culture = LocalizeCore.CurrentCulture ?? System.Globalization.CultureInfo.CurrentUICulture.Name;
					string[] templates;
					if (culture.StartsWith("ja"))
						templates = new[] { "今日は{0}だよ〜可愛い色だね！", "哇！今日のラッキーカラーは{0}だよ！" };
					else if (culture.StartsWith("en"))
						templates = new[] { "Look! Today's lucky color is {0} meow!", "It's {0} today — nice choice meow!" };
                    else if (culture.StartsWith("zh-Hant"))
                        // 繁体，使用“主人”称呼更像桌宠风格
                        templates = new[] { "主人，今天的幸運色是{0}喵～", "嗷！今日幸運色：{0}！好漂亮喵～ 主人你也覺得嗎？" };
                    else // 默认简体中文
                        // 简体中文使用“主人”称呼
                        templates = new[] { "主人，今天适合穿{0}色！好好看喵～", "主人今天的幸运色是{0}，记得搭配一下喵～" };
                    // 使用当天种子选择模板，保证同一天一致
                    var rnd = new Random(DateTime.Now.Date.GetHashCode());
                    var msgTemplate = templates[rnd.Next(templates.Length)];
                    // 如果模板中包含特殊占位符 {host} 则替换hostName
                    if (msgTemplate.Contains("{host}") && !string.IsNullOrWhiteSpace(hostName))
                        msgTemplate = msgTemplate.Replace("{host}", hostName);
                    msg = string.Format(msgTemplate, _todaysLuckyColorName ?? "");
                }

                // 使用当前选中宠物的显示名作为说话者，优先使用 pet loader 提供的 PetName（以跟随宠物自身语言）
                var rawPetName = GameSavesData?.GameSave?.Name ?? Main?.Core?.Save?.Name ?? "VPet";
                var petloader = Pets?.Find(x => x.Name == Set?.PetGraph) ?? Pets?.FirstOrDefault();
                var petName = petloader?.PetName ?? rawPetName;
                if (!string.IsNullOrWhiteSpace(msg))
                    Main?.MsgBar?.Show(petName, msg);
                else
                    Main?.MsgBar?.Show(petName, (_todaysLuckyColorName ?? "").ToString());
			}
			catch { }
		}

		private void GenerateAndApplyLuckyColor()
		{
			var culture = LocalizeCore.CurrentCulture ?? System.Globalization.CultureInfo.CurrentUICulture.Name;
			var colorDefs = new List<Color> {
				Function.HEXToColor("#3EB489"), // 薄荷
				Function.HEXToColor("#FF6F61"), // 珊瑚
				Function.HEXToColor("#87CEEB"), // 天空
				Function.HEXToColor("#9B72AA"), // 薰衣
				Function.HEXToColor("#FFD966"), // 柠檬
			};
			string[] names = new[] { "薄荷绿", "珊瑚粉", "天空蓝", "薰衣紫", "柠檬黄" };
			if (culture != null)
			{
				if (culture.StartsWith("zh-Hant"))
					names = new[] { "薄荷綠", "珊瑚粉", "天空藍", "薰衣紫", "檸檬黃" };
				else if (culture.StartsWith("ja"))
					names = new[] { "ミントグリーン", "コーラルピンク", "スカイブルー", "ラベンダー", "レモンイエロー" };
				else if (culture.StartsWith("en"))
					names = new[] { "Mint", "Coral", "Sky", "Lavender", "Lemon" };
			}
			var r = new Random(DateTime.Now.Date.GetHashCode());
			int idx = r.Next(colorDefs.Count);
			var baseC = colorDefs[idx];
			_todaysLuckyColor = baseC;
			_todaysLuckyColorName = names[idx];
			// 写入资源
			Application.Current.Resources["PrimaryTrans"] = new SolidColorBrush(Color.FromArgb(204, baseC.R, baseC.G, baseC.B));
			Application.Current.Resources["PrimaryTrans4"] = new SolidColorBrush(Color.FromArgb(44, baseC.R, baseC.G, baseC.B));
			Application.Current.Resources["PrimaryTransA"] = new SolidColorBrush(Color.FromArgb(170, baseC.R, baseC.G, baseC.B));
			Application.Current.Resources["PrimaryTransE"] = new SolidColorBrush(Color.FromArgb(238, baseC.R, baseC.G, baseC.B));
		}
			...
			await Dispatcher.InvokeAsync(() =>
			  {
                  ...
                  // 使用本地化的标题键，确保在切换语言时菜单文本也跟随变化
                  string luckyTitle = "LuckyColor_Title".Translate();
                  if (string.IsNullOrWhiteSpace(luckyTitle) || luckyTitle == "LuckyColor_Title")
                  {
                      var culture = LocalizeCore.CurrentCulture ?? System.Globalization.CultureInfo.CurrentUICulture.Name;
                      if (culture.StartsWith("ja")) luckyTitle = "本日のラッキーカラー";
                      else if (culture.StartsWith("en")) luckyTitle = "Today's Lucky Color";
                      else if (culture.StartsWith("zh-Hant")) luckyTitle = "今日幸運色";
                      else luckyTitle = "今日幸运色";
                  }
                  m_menu.Items.Add(new MenuItem(luckyTitle, null, (s, e) => { Dispatcher.Invoke(() => ShowLuckyColor()); }));
