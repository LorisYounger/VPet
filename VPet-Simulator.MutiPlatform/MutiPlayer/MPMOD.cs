...
/// 跨平台: 原文复制自 VPet-Simulator.Windows/MutiPlayer/MPMOD.cs; 路径拼接用 Path.Combine (Linux 上 '\' 不是分隔符), 其余逐行相同
...
#endif
        var Path = directory;
        LpsDocument modlps = new LpsDocument(File.ReadAllText(System.IO.Path.Combine(directory.FullName, "info.lps")));
