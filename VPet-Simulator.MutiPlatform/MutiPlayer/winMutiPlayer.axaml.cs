...
/// </summary>
/// 跨平台: 原文复制自 VPet-Simulator.Windows/MutiPlayer/winMutiPlayer.xaml.cs. WindowX→VPetWindow, ImageSource→Bitmap,
/// IGraph→IAvaloniaGraph, Visibility→IsVisible, Dispatcher.Invoke→Dispatcher.UIThread, Switch 的 Checked/Unchecked 合成
/// IsCheckedChanged 后按 IsChecked 分流; 页签上方那块 (pu:TabControlHelper.FrontControl) 写在 TabControl 的模板里, 其中的控件
/// 在模板套上时从命名域里取. 其余逐行相同
public partial class winMutiPlayer : VPetWindow, IMPWindows
{
    //跨平台: 这些控件在 TabControl 的模板里 (见 winMutiPlayer.axaml), 由 tabControl_TemplateApplied 取出
    ImageBrush HostHead = null!;
    Run hostPet = null!;
    Run hostName = null!;
    TextBox lbLid = null!;
    StackPanel panFixID = null!;
    TextBox lbLFix = null!;
    Button btnRels = null!;
    Switch swAllowJoin = null!;
    Switch swAllowTouch = null!;
    private void tabControl_TemplateApplied(object? sender, TemplateAppliedEventArgs e)
    {
        HostHead = (ImageBrush)e.NameScope.Get<Border>("HostHeadBorder").Background!;
        hostPet = e.NameScope.Get<TextBlock>("tbHostPet").Inlines!.OfType<Run>().First(x => x.Name == "hostPet");
        hostName = e.NameScope.Get<TextBlock>("tbHostName").Inlines!.OfType<Run>().First(x => x.Name == "hostName");
        lbLid = e.NameScope.Get<TextBox>("lbLid");
        panFixID = e.NameScope.Get<StackPanel>("panFixID");
        lbLFix = e.NameScope.Get<TextBox>("lbLFix");
        btnRels = e.NameScope.Get<Button>("btnRels");
        btnRels.Click += btnRels_Click;
        swAllowJoin = e.NameScope.Get<Switch>("swAllowJoin");
        swAllowJoin.IsCheckedChanged += swAllowJoin_Checked;
        swAllowTouch = e.NameScope.Get<Switch>("swAllowTouch");
        swAllowTouch.IsCheckedChanged += swAllowTouch_Checked;
    }
        ...
        _ = Task.Run(async () =>
        {
            if (int.TryParse(await mw.GetVPetRoom("SteamRoomGetFixID", lobbyid: lb.Id.Value), out int FixID))
            {
                Dispatcher.Invoke(() =>
                {
                    ...
                    btnRels.IsVisible = false;
                    panFixID.IsVisible = true;
                });
            }
        });
    }
    ...
    //跨平台: BitmapSource.Create + PngBitmapEncoder 换成 WriteableBitmap, 像素格式直接用 Rgba8888 所以不用换通道
    public static Bitmap ConvertToImageSource(Steamworks.Data.Image? img)
    {
        if (img == null)
        {
            return ImageResources.NewSafeBitmapImage("avares://VPet-Simulator.MutiPlatform/Res/vpeticon.png");
        }
        var image = img.Value;
        int stride = (int)((image.Width * 32 + 7) / 8); // 32 bits per pixel
        var bitmap = new WriteableBitmap(new Avalonia.PixelSize((int)image.Width, (int)image.Height), new Avalonia.Vector(96, 96), PixelFormat.Rgba8888, AlphaFormat.Unpremul);
        using (var buffer = bitmap.Lock())
        {
            for (int y = 0; y < image.Height; y++)
                Marshal.Copy(image.Data, y * stride, buffer.Address + y * buffer.RowBytes, stride);
        }
        return bitmap;
    }
    ...
    public void ShowLobbyInfo()
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            Dispatcher.Invoke(() =>
            {
                ...
                HostHead.Source = ConvertToImageSource(img);
            });
    ...
    private void SteamMatchmaking_OnLobbyDataChanged(Lobby lobby)
    {
        if (lb.Id == lobby.Id)
        {
            ...
            {
                Task.Run(() => MessageBoxX.Show("访客表已被房主{0}关闭".Translate(lb.Owner.Name)));//温柔的谎言
                lb.Leave();
                lb = default(Lobby);
                Close();
            }
    ...
    private void SteamMatchmaking_OnLobbyMemberLeave(Lobby lobby, Friend friend)
    {
        ...
        if (friend.Id == HostID)
        {
            Task.Run(() => MessageBoxX.Show("访客表已被房主{0}关闭".Translate(friend.Name)));
            lb = default(Lobby);
            Close();
        }
        else
    ...
    public void Log(string message)
    {
        ...
        Dispatcher.Invoke(() => tbLog.Text += $"[{DateTime.Now.ToShortTimeString()}]{message}\n");
    }
    ...
    //跨平台: Avalonia 的开关只有 IsCheckedChanged, 在入口按 IsChecked 分流
    private void swAllowJoin_Checked(object? sender, RoutedEventArgs e)
    {
        if (swAllowJoin.IsChecked != true)
        {
            swAllowJoin_Unchecked(sender, e);
            return;
        }
    ...
    public void DisplayNOCALTouchHead()
    {
        if (mw.Main.DisplayType.Type == GraphType.Touch_Head)
        {
            if (mw.Main.DisplayType.Animat == AnimatType.A_Start)
                ...
                if (Dispatcher.Invoke(() => mw.Main.PetGrid.Tag) is IAvaloniaGraph ig && ig.GraphInfo.Type == GraphType.Touch_Head && ig.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig.SetContinue();
                    return;
                }
                else if (Dispatcher.Invoke(() => mw.Main.PetGrid2.Tag) is IAvaloniaGraph ig2 && ig2.GraphInfo.Type == GraphType.Touch_Head && ig2.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig2.SetContinue();
                    return;
                }
    ...
    public void DisplayNOCALTouchBody()
    {
        if (mw.Main.DisplayType.Type == GraphType.Touch_Body)
        {
            if (mw.Main.DisplayType.Animat == AnimatType.A_Start)
                ...
                if (Dispatcher.Invoke(() => mw.Main.PetGrid.Tag) is IAvaloniaGraph ig && ig.GraphInfo.Type == GraphType.Touch_Body && ig.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig.SetContinue();
                    return;
                }
                else if (Dispatcher.Invoke(() => mw.Main.PetGrid2.Tag) is IAvaloniaGraph ig2 && ig2.GraphInfo.Type == GraphType.Touch_Body && ig2.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig2.SetContinue();
                    return;
                }
    ...
    {
        if (mw == null) return;
        if (swAllowTouch.IsChecked != true)
        {
            swAllowTouch_Unchecked(sender, e);
            return;
        }
