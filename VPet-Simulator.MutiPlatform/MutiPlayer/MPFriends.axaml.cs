...
using ToolBar = VPet_Simulator.Core.MutiPlatform.Display.ToolBar;

namespace VPet_Simulator.MutiPlatform;
/// <summary>
/// MPFriends.xaml 的交互逻辑
/// </summary>
/// 跨平台: 原文复制自 VPet-Simulator.Windows/MutiPlayer/MPFriends.xaml.cs. WindowX→Window (与 MainWindow 同), ImageSource→Bitmap,
/// IGraph→IAvaloniaGraph, Visibility→IsVisible, Dispatcher.Invoke→Dispatcher.UIThread, Panuon 的 Watermark→原生 Watermark,
/// Main 的工具栏/消息栏由 Main 自己建 (跨平台 Main 的构造函数里就挂好了), 其余逐行相同
public partial class MPFriends : Window, IMPFriend
{
    //跨平台: 与 MainWindow 相同的四个换算属性, 让 MPController 的代码与 Windows 版逐字相同
    ...
    public MPFriends(winMutiPlayer wmp, MainWindow mw, Lobby lb, Friend friend)
    {
        ...
        try
        {
            ...
            //跨平台: 窗口尺寸也按倍率给 (WPF 那边靠 SizeToContent)
            Width = 500 * mw.Set.ZoomLevel;
            Height = 500 * mw.Set.ZoomLevel;
            double L = 0, T = 0;
            ...
            Dispatcher.Invoke(() =>
            {
                if (tmp == "True")
                {
                    if (hashcheckimg == null)
                    {
                        ...
                        ToolTip.SetTip(hashcheckimg, "是没有修改过存档/使用超模MOD的玩家专属标志".Translate());
        ...
        if (lobby.Id == lb.Id && friend.Id == this.friend.Id)
        {
            ...
            Dispatcher.Invoke(() =>
            {
                if (!string.IsNullOrEmpty(tmp))
                {
                    ...
                    Main.ToolBar.tfun.IsVisible = false;
                }
    ...
    public async Task GameLoad(List<DirectoryInfo> Path)
    {
        ...
        await Dispatcher.InvokeAsync(() => LoadingText.Content = "Loading MOD");
        //加载mod
        foreach (DirectoryInfo di in Path)
        {
            if (!File.Exists(System.IO.Path.Combine(di.FullName, "info.lps")))
                continue;
            await Dispatcher.InvokeAsync(() => LoadingText.Content = $"Loading MOD: {di.Name}");
            MPMODs.Add(new MPMOD(di, this));
        }

        await Dispatcher.InvokeAsync(() => LoadingText.Content = "尝试加载游戏MOD".Translate());
        ...
        await Dispatcher.InvokeAsync(() =>
        {
            ...
            //跨平台: 跨平台的 PetLoader.Graph 没有 Dispatcher 参数; 消息栏/工具栏由 Main 的构造函数建好并挂在 UIGrid 上, 不用再建
            ...
            //清空资源
            //跨平台: 主题颜色写在 App 的资源上 (ThemeLoader.Apply), 控件本来就从那里取, 不用逐个挂
            ...
            LoadingText.VerticalAlignment = Avalonia.Layout.VerticalAlignment.Top;
    ...
    public void DisplayNOCALTouchHead()
    {
        if (Main.DisplayType.Type == GraphType.Touch_Head)
        {
            if (Main.DisplayType.Animat == AnimatType.A_Start)
                ...
                if (Dispatcher.Invoke(() => Main.PetGrid.Tag) is IAvaloniaGraph ig && ig.GraphInfo.Type == GraphType.Touch_Head && ig.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig.SetContinue();
                    return;
                }
                else if (Dispatcher.Invoke(() => Main.PetGrid2.Tag) is IAvaloniaGraph ig2 && ig2.GraphInfo.Type == GraphType.Touch_Head && ig2.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig2.SetContinue();
                    return;
                }
    ...
    public void DisplayNOCALTouchBody()
    {
        if (Main.DisplayType.Type == GraphType.Touch_Body)
        {
            if (Main.DisplayType.Animat == AnimatType.A_Start)
                ...
                if (Dispatcher.Invoke(() => Main.PetGrid.Tag) is IAvaloniaGraph ig && ig.GraphInfo.Type == GraphType.Touch_Body && ig.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig.SetContinue();
                    return;
                }
                else if (Dispatcher.Invoke(() => Main.PetGrid2.Tag) is IAvaloniaGraph ig2 && ig2.GraphInfo.Type == GraphType.Touch_Body && ig2.GraphInfo.Animat == AnimatType.B_Loop)
                {
                    ig2.SetContinue();
                    return;
                }
    ...
    private void cbTalk_SelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        ...
        {
            case 0:
                tbTalk.PlaceholderText = "和{0}悄悄说".Translate(friend.Name);
                break;
            case 1:
                tbTalk.PlaceholderText = "和{0}说".Translate(friend.Name);
                break;
            case 2:
                tbTalk.PlaceholderText = "和大家说";
                break;
        }
    }
    ...
    public void ReSetLocal()
    {
        Main?.CleanState();
        Main?.DisplayToNomal();
        Left = (mw.MWController.PrimaryScreen.Width - ActualWidth) / 2;
        Top = (mw.MWController.PrimaryScreen.Height - ActualHeight) / 2;
    }
