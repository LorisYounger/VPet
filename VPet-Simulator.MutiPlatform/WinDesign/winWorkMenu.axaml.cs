...
/// </summary>
/// 跨平台: 原文复制自 VPet-Simulator.Windows/WinDesign/winWorkMenu.xaml.cs; WindowX→VPetWindow, Visibility→IsVisible,
/// Dispatcher.BeginInvoke→Dispatcher.UIThread.Post, ComboBoxHelper/TextBoxHelper.SetWatermark→PlaceholderText/Watermark,
/// PreviewMouseLeftButtonDown→构造函数里 AddHandler(Tunnel), 其余逐行相同
public partial class winWorkMenu : VPetWindow
{
    MainWindow mw;
    List<Work> ws;
    List<Work> ss;
    ...
    public winWorkMenu(MainWindow mw, Work.WorkType type)
    {
        ...
        tbtnAgencyJob.AddHandler(PointerPressedEvent, tbtnAgency_PreviewMouseLeftButtonDown, RoutingStrategies.Tunnel);
        tbtnAgencyTraning.AddHandler(PointerPressedEvent, tbtnAgency_PreviewMouseLeftButtonDown, RoutingStrategies.Tunnel);
        this.mw = mw;
        ...
        detailTypes.PlaceholderText = "---" + "请选择".Translate() + "---";

        combTaskType.PlaceholderText = "---" + "请选择".Translate() + "套餐".Translate() + "---";
    ...
    public void ShowWork(Work work)
    {
        ...
        {
            WorkViewImage.Source = ImageResources.NewSafeBitmapImage(source);
        }
    ...
    private void LsbCategory_SelectionChanged(object? sender, SelectionChangedEventArgs e)
    {
        Dispatcher.UIThread.Post(() =>
        {
            ...
            gdWork.IsVisible = true;
            gdSchedule.IsVisible = false;
            ...
            switch (LsbCategory.SelectedIndex)
            {
                case 0:
                    ...
                    detailTypes.PlaceholderText = "---" + "请选择".Translate() + "工作".Translate() + "---";
                    tbDIYName.Watermark = "自定义{0}名称".Translate("工作".Translate());
                    break;
                case 1:
                    detailTypes.ItemsSource = _studyDetails;
                    btnStart.Content = "开始学习".Translate();
                    detailTypes.PlaceholderText = "---" + "请选择".Translate() + "学习".Translate() + "---";
                    tbDIYName.Watermark = "自定义{0}名称".Translate("学习".Translate());
                    break;
                case 2:
                    detailTypes.ItemsSource = _playDetails;
                    btnStart.Content = "开始玩耍".Translate();
                    detailTypes.PlaceholderText = "---" + "请选择".Translate() + "玩耍".Translate() + "---";
                    tbDIYName.Watermark = "自定义{0}名称".Translate("玩耍".Translate());
                    break;
                case 3:
                    detailTypes.ItemsSource = _starDetails;
                    btnStart.Content = "开始活动".Translate();
                    detailTypes.PlaceholderText = "---" + "请选择".Translate() + "---";
                    tbDIYName.Watermark = "自定义{0}名称".Translate("");
                    break;
                case 4:
                    gdWork.IsVisible = false;
                    gdSchedule.IsVisible = true;
                    return;
            }
    ...
    private void wDouble_ValueChanged(object? sender, RangeBaseValueChangedEventArgs e)
    {
    ...
    private void NumberInput_ValueChanged(object? sender, NumericUpDownValueChangedEventArgs e)
    {
        CalculateSceduleTime();
    }
    int sworkTime = 0;
    ...
    //跨平台: Avalonia 的 ToggleButton 只有 IsCheckedChanged (勾上取消都触发), Windows 版只接 Checked, 这里取消的跳过
    ...
    private void tbtnAgency_PreviewMouseLeftButtonDown(object? sender, PointerPressedEventArgs e)
    {
    ...
    private void sliderTaskLevel_ValueChanged(object? sender, RangeBaseValueChangedEventArgs e)
    {
        if (!AllowChange) return;
        nowselefullDisplay();
    }
