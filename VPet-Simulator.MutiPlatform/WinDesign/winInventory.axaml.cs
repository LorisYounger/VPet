...
namespace VPet_Simulator.MutiPlatform
{
    ...
    /// </summary>
    /// 跨平台: 原文复制自 VPet-Simulator.Windows/WinDesign/winInventory.xaml.cs; WindowX→VPetWindow,
    /// Visibility→IsVisible, Dispatcher.BeginInvoke→Dispatcher.UIThread.Post, 其余逐行相同
    public partial class winInventory : VPetWindow
    {
        MainWindow mw;
        ...
        public new void Show()
        {
            ...
            Dispatcher.UIThread.Post(UpdateList, DispatcherPriority.Loaded);
        ...
        private void UpdateList()
        {
            ...
            if (!items.Any())
                TbNone.IsVisible = true;
            else
                TbNone.IsVisible = false;
        }
        ...
        /// 总价值文本加载完成事件处理
        /// </summary>
        //跨平台: Run 没有 Loaded 事件, 挂在外层 TextBlock 上, 从它的 Inlines 里按名字找
        private void rTotalValue_Loaded(object? sender, RoutedEventArgs e)
        {
            rTotalValue = (sender as TextBlock)?.Inlines?.OfType<Run>().FirstOrDefault(x => x.Name == "rTotalValue");
        ...
        /// 物品格子单击事件处理
        /// </summary>
        //跨平台: MouseLeftButtonUp 换成 PointerReleased, 只认左键
        private void CellRoot_MouseLeftButtonUp(object? sender, PointerReleasedEventArgs e)
        {
        ...
        private void DisplayDetail(Item item)
        {
            ...
            if (item.IsSingle)
            {
                spUseEnter.IsVisible = false;
                BorderIsSingle.IsVisible = true;
            }
            else
            {
                spUseEnter.IsVisible = true;
                BorderIsSingle.IsVisible = false;
            }
        }
