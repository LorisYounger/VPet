...
namespace VPet_Simulator.MutiPlatform
{
    ...
    /// </summary>
    /// 跨平台: 原文复制自 VPet-Simulator.Windows/WinDesign/winBetterBuy.xaml.cs; WindowX→VPetWindow, Switch→shell:Switch,
    /// Dispatcher.Invoke→Dispatcher.UIThread.Invoke, Visibility→IsVisible, 其余逐行相同
    public partial class winBetterBuy : VPetWindow
    {
        ...
        public void Show(Food.FoodType type)
        {
            ...
            nibuytimes.Maximum = (decimal)Math.Max(2, mw.Core.Save!.StrengthMax / 100);
        ...
        {
            var rows = Math.Max(0, (int)Math.Floor(IcCommodity.Bounds.Height / 150d));
            if (rows != _rows)
            {
                _rows = rows;
                Search();
        ...
        {
            rMoney = (sender as TextBlock)?.Inlines?.OfType<Run>().FirstOrDefault(x => x.Name == "rMoney");
            rMoney!.Text = mw.Core.Save!.Money.ToString("f2");
        }
        ...
        private void Switch_AutoBuy_Checked(object? sender, RoutedEventArgs e)
        {
            ...
            {
                mw.Set.AutoBuy = true;
                _puswitchautogift!.IsVisible = true;
            }
            else
            {
                mw.Set.AutoBuy = false;
                _puswitchautogift!.IsVisible = false;
            }
        }
        ...
        private void nibuytimes_ValueChanged(object? sender, NumericUpDownValueChangedEventArgs e)
        {
            if (!AllowChange)
                return;
