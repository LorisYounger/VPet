//跨平台: 原文复制自 VPet-Simulator.Windows/Function/Setting.cs, 只换了本地化/Point/Visibility 三处, 末尾补了设置文件的读写
using LinePutScript;
...
namespace VPet_Simulator.MutiPlatform
{
    ...
    internal class Setting : LPS_D, ISetting
    {
        ...
        /// 读设置; 读不了就用默认值并给出原因
        /// </summary>
        public static Setting Load(MainWindow mw, string prefix, out string? warning)
        {
            warning = null;
            var path = MultiPetStore.SettingPath(AppPaths.DataRoot, prefix);
            if (!File.Exists(path))
                return new Setting(mw, "") { Prefix = prefix };
            try
            {
                return new Setting(mw, File.ReadAllText(path)) { Prefix = prefix };
            }
            catch (Exception ex)
            {
