//跨平台: 原文复制自 VPet-Simulator.Windows/Function/CoreMOD.cs. 差异只有三处: 宿主自带 dll 的名单、去掉旧式 MainPlugin 那条路、
//引用了 WPF 那层的插件由 PluginClassifier 判出来记进 ErrorMessage 而不是去加载
//路径拼接用 Path.Combine (Linux 上 '\' 不是分隔符)
using LinePutScript;
...
namespace VPet_Simulator.MutiPlatform
{
    internal class CoreMOD : IModInfo
    {
        ...
        //跨平台: 这份名单是宿主自己带的 dll, 插件目录里同名的一律跳过 (Windows 版那份是 WPF 宿主的)
        ...
        public CoreMOD(DirectoryInfo directory, MainWindow mw)
        {
            ...
            var meta = ModInfoReader.Parse(directory)
                ?? throw new FileNotFoundException("找不到 info.lps", System.IO.Path.Combine(directory.FullName, "info.lps"));
            ...
            foreach (DirectoryInfo di in Path.EnumerateDirectories())
            {
                switch (di.Name.ToLowerInvariant())
                {
                    case "theme":
                        Tag.Add("theme");
                        if (Directory.Exists(System.IO.Path.Combine(di.FullName, "fonts")))
                            foreach (var str in Directory.EnumerateFiles(System.IO.Path.Combine(di.FullName, "fonts"), "*.ttf"))
                            {
                        ...
                        foreach (FileInfo fi in di.EnumerateFiles("*.lps"))
                        {
                            ...
                            mw.Themes.Add(tmp);
                            //加载图片包
                            DirectoryInfo tmpdi = new DirectoryInfo(System.IO.Path.Combine(di.FullName, tmp.Image));
                            if (tmpdi.Exists)
                            {
                        ...
                        foreach (FileInfo fi in di.EnumerateFiles("*.lps"))
                        {
                            ...
                            if (lps.First()!.Name.ToLowerInvariant() == "pet")
                            {
                                ...
                                else
                                {
                                    ...
                                    var dis = new DirectoryInfo(System.IO.Path.Combine(di.FullName, lps.First()!["path"].Info));
                                    if (dis.Exists && dis.GetDirectories().Length > 0)
                                        Tag.Add("pet");
                                    p.path.Add(System.IO.Path.Combine(di.FullName, lps.First()!["path"].Info));
                                    p.Config.Set(lps);
                                }
                            }
                        }
                    ...
                    case "plugin":
                        ...
                        if (File.Exists(System.IO.Path.Combine(di.FullName, "load.lps")))
                            loadfile = new LpsDocument(File.ReadAllText(System.IO.Path.Combine(di.FullName, "load.lps")));
                        string authtype = "";
                        foreach (FileInfo tmpfi in di.EnumerateFiles("*.dll"))
                        {
                            ...
                            try
                            {
                                ...
                                //跨平台: 只引用 Windows.Interface 那层的旧插件在这里跑不起来, 认出来就报告, 不去加载
                                if (PluginClassifier.Classify(tmpfi.FullName) == PluginKind.LegacyWindows)
                                {
                                    ErrorMessage = $"{tmpfi.Name} 是 Windows 版专用的插件, 跨平台版加载不了";
                                    SuccessLoad = false;
                                    continue;
                                }
                                ...
                                {
                                    //跨平台: 只认统一契约的插件. 旧式 MainPlugin 是 WPF 的, 在别的平台跑不起来,
                                    //引用了 Windows.Interface 的 dll 由 PluginClassifier 认出来后记进警告
                                    if (exportedType.BaseType == typeof(VPet_Simulator.Unified.Interface.UnifiedPlugin))
                                    {
        ...
        {
            LpsDocument modlps = new LpsDocument(File.ReadAllText(System.IO.Path.Combine(Path.FullName, "info.lps")));
