...
// 一些桌宠功能的实现函数
// 跨平台: 原文复制自 VPet-Simulator.Windows/MainWindow_Function.cs, 末尾多出的是跨平台独有的语音转发
public partial class MainWindow : IMainWindow
{
    ...
    /// 所以语音由 MOD 实现, 宿主只负责转发. 插件可能在 Main 建好之前就登记了播放器,
    /// 先存着, 等桌宠建好再交过去
    private IVoicePlayer? voicePlayer;

    internal void SetVoicePlayer(IVoicePlayer player)
    {
        voicePlayer = player;
        //交给 Main: 消息栏的口型同步要从那里读还剩多久
        if (Main != null)
            Main.VoicePlayer = player;
    }
    ...
    {
        if (Main != null && voicePlayer != null)
            Main.VoicePlayer = voicePlayer;
    }
}
