...
public partial class ActivityLog
{
    ...
    /// 转换成玩家可读的字符串
    /// </summary>
    /// <param name="save">存档, 用来把文本里的占位符换成实际值</param>
    /// 这是本次迁移里唯一允许新增的公开成员: 跨平台侧没有 Main, 但活动日志
    /// 是设置窗口和年度报告都要显示的东西, 必须有一条不依赖界面的路。
    public string ToString(IGameSave save)
    {
        return $"[{Time.ToShortTimeString()}] {string.Format(SaveTextTemplate.Convert(("al_" + Type).Translate(), save), Description.Split('|'))}";
    }
}
