...
using System.IO;
using System.Linq;
using static VPet_Simulator.Core.GraphTypeNames;
...
namespace VPet_Simulator.Core
{
    ...
    public class GraphInfo
    {
        ...
        public GraphInfo(FileSystemInfo path, ILine info)
        {
            ...
            // 动画的类型/状态/动作是从目录层级推断出来的, 所以要先把路径分隔符
            // 拍平成下划线. 这里必须同时处理两种分隔符: Windows 上 FullName 用
            // 反斜杠, Linux/macOS 上用正斜杠, 只处理反斜杠会导致非 Windows 平台
            // 上整条路径被当成一个片段, 动画类型全部识别错.
            // Windows 上 FullName 不会产生正斜杠, 因此多这一次替换不改变原有行为.
            var path_name = pn.Replace('\\', '_').Replace('/', '_').Split('_').ToList();
